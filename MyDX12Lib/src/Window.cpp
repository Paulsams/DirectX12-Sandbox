...
void Window::OnResize(ResizeEventArgs& eventArgs)
{
    ...
    if (m_ClientWidth != eventArgs.Width || m_ClientHeight != eventArgs.Height)
    {
        m_ClientWidth = std::max(1, eventArgs.Width);
        m_ClientHeight = std::max(1, eventArgs.Height);
