...
ComPtr<IDXGIAdapter4> Application::GetAdapter(const bool bUseWarp)
{
...
std::shared_ptr<CommandQueue> Application::GetCommandQueue(D3D12_COMMAND_LIST_TYPE type) const
{
    switch (type)
    {
        case D3D12_COMMAND_LIST_TYPE_DIRECT:
            return std::shared_ptr(m_DirectCommandQueue);
        case D3D12_COMMAND_LIST_TYPE_COMPUTE:
            return std::shared_ptr(m_ComputeCommandQueue);
        case D3D12_COMMAND_LIST_TYPE_COPY:
            return std::shared_ptr(m_CopyCommandQueue);
        default:
            ...
            case WM_MOUSEMOVE:
                {
                    ...
                    static int lastMousePositionX = 0;
                    static int lastMousePositionY = 0;
                    ...
                    int relX = isFirstFrame ? 0 : x - lastMousePositionX;
                    int relY = isFirstFrame ? 0 : y - lastMousePositionY;
            ...
            case WM_MOUSEWHEEL:
                {
                    ...
                    float zDelta    = static_cast<short>(HIWORD(wParam)) / static_cast<float>(WHEEL_DELTA);
                    ...
                    MouseWheelEventArgs mouseWheelEventArgs(zDelta, lButton, mButton,
                        rButton, control, shift,
                        static_cast<int>(clientToScreenPoint.x), static_cast<int>(clientToScreenPoint.y));
